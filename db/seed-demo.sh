#!/usr/bin/env bash
# Reemplaza el set de demostración (prendas LS-1000xx, boletas V-D…, gastos del período).
...
if [[ "$missing" -ne 0 ]]; then
  echo "Genera las 16 fotos demo (una por prenda LS-1000xx) antes de sembrar."
  exit 1
fi

echo "ATENCIÓN: este script borra y vuelve a cargar datos DEMO"
echo "  (productos LS-1000xx, ventas V-D…, gastos del local jun–ago 2026)."
echo "No toca admin@lscala.cl. Archiva prendas de prueba (QA / LS-000…)."
